...
- Optional integration with Crynano's Mod Configuration Menu: when subscribed, `stationTrade`, `tradeConfirm`, `autoUnlockTech`, and `debugTradeLayout` can be changed from the in-game mod settings page. Changes apply immediately and are persisted to the mod's own `mcm_settings.txt` (works even without MCM next time).
...
- Adds the configurable `C` shortcut on the spaceship screen.
...
- `shortcutKey=C`: Unity `KeyCode` for the spaceship shortcut; use `None` to disable it.
...
With Crynano's Mod Configuration Menu subscribed, `stationTrade`, `tradeConfirm`, `autoUnlockTech`, and `debugTradeLayout` can also be changed from the in-game mod settings page; changes apply immediately and are written to `mcm_settings.txt`.
